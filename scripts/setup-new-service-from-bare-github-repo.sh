...
print_step "Renaming service directory from mgraph_ai_service_html to $SERVICE_NAME_UNDERSCORES..."
if [ -d "mgraph_ai_service_html" ]; then
    mv mgraph_ai_service_html "$SERVICE_NAME_UNDERSCORES"
    print_success "Service directory renamed"
else
    print_error "mgraph_ai_service_html directory not found!"
    exit 1
fi
...
# Replace "MGraph AI Service Html" with the new service title
replace_in_files "MGraph AI Service Html" "$SERVICE_NAME_TITLE" "service title"

# Replace "mgraph_ai_service_html" with the new package name
replace_in_files "mgraph_ai_service_html" "$SERVICE_NAME_UNDERSCORES" "Python package name"

# Replace "MGraph-AI__Service__Html" with the new repository name
replace_in_files "MGraph-AI__Service__Html" "$REPO_NAME" "repository name"
